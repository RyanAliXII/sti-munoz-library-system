...
func (ctrler * User)GetUserTypes(ctx *gin.Context){
	...
	if filter.HasProgram {
		...
		if err != nil {
			ctrler.services.Logger.Error(err.Error(), applog.Error("getUserTypesErr"))
		}
...
func (ctrler * User)NewUserType(ctx *gin.Context){
	...
	if err != nil {
		ctrler.services.Logger.Error(err.Error(), applog.Error("NewUserType"))
...
func (ctrler * User)NewUserProgram(ctx *gin.Context){
	...
	if err != nil {
		ctrler.services.Logger.Error(err.Error(), applog.Error("NewUserProgram"))
		ctx.JSON(httpresp.Fail500(nil, "Unknown error occured."))
		return
	}
	ctx.JSON(httpresp.Success200(gin.H{
...
func (ctrler * User)UpdateUserProgram(ctx *gin.Context){
	...
	if err != nil {
		ctrler.services.Logger.Error(err.Error(), applog.Error("UpdateProgram"))
		ctx.JSON(httpresp.Fail500(nil, "Unknown error occured."))
		return
	}
	ctx.JSON(httpresp.Success200(gin.H{
...
func (ctrler * User)GetUserProgramsAndStrandsByType(ctx *gin.Context){
	...
	if err != nil {
		ctrler.services.Logger.Error(err.Error(), applog.Error("GetUserProgramsAndStrandsByTypeErr"))
	}
	ctx.JSON(httpresp.Success200(gin.H{
		"programs": programs,
