...
display_name, 'email', email, 'givenName', account.given_name, 'surname', account.surname) as client, account.search_vector  as account_search_vector,
...
(case when user_type.name is null then '' else user_type.name end) as user_type,
...
LEFT JOIN system.user_program on account.program_id = user_program.id
LEFT JOIN
