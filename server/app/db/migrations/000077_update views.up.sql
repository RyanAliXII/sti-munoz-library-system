--CREATE BOOK VIEW
	...
	LEFT JOIN circulation.online_borrowed_book as obb on accession.id = obb.accession_id and obb.status != 'cancelled' and obb.status != 'returned' and obb.status != 'unreturned'
	GROUP BY 
	book.id,
	source_of_fund.id,
	section.id,
