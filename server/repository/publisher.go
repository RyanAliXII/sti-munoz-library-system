...
func (repo *Publisher) Get(filter * filter.Filter) []model.Publisher {
	...
	if selectErr != nil {
		logger.Error(selectErr.Error(), applog.Function(GET_PUBLISHERS))
	}
	return publishers
}
...
func (repo *Publisher) New(publisher model.Publisher) (model.Publisher, error) {
	...
	if insertErr != nil {
		logger.Error(insertErr.Error(), applog.Function(NEW_PUBLISHER), applog.Error("insertErr"))
...
func (repo *Publisher) Delete(id string) error {
	...
	if prepareErr != nil {
		logger.Error(prepareErr.Error(), zap.String("publisherId", id), applog.Function(DELETE_PUBLISHER))
		return prepareErr
	}
	...
	if getAffectedErr != nil || affected > SINGLE_RESULT {
		logger.Warn(getAffectedErr.Error(), zap.String("publisherId", id), applog.Function(DELETE_PUBLISHER))
	}
	logger.Info("model.Publisher Deleted", zap.String("publisherId", id), applog.AffectedRows(affected), applog.Function(DELETE_PUBLISHER))
	return deleteErr
}
func (repo *Publisher) Update(id string, publisher model.Publisher) error {
	...
	if prepareErr != nil {
		logger.Error(prepareErr.Error(), zap.String("publisherId", id), applog.Function(UPDATE_PUBLISHER))
		return prepareErr
	}
	updateResult, updateErr := updateStmt.Exec(publisher.Name, id)
	if updateErr != nil {
		logger.Error(updateErr.Error(), zap.String("publisherId", id), applog.Function(UPDATE_PUBLISHER))
		return updateErr
	}
	affected, getAffectedErr := updateResult.RowsAffected()
	const SINGLE_RESULT = 1
	if getAffectedErr != nil || affected > SINGLE_RESULT {
		logger.Warn(getAffectedErr.Error(), zap.String("publisherId", id), applog.Function(UPDATE_PUBLISHER))
	}
	logger.Info("model.Publisher Updated", zap.String("publisherId", id), applog.AffectedRows(affected), applog.Function(UPDATE_PUBLISHER))
	return updateErr
}
